            ...
            for (int i = 0; i < lstArquivos.Count; i++)
            {
                ...
                using (FileStream fs = File.Open(lstArquivos[i], FileMode.Open, FileAccess.ReadWrite, FileShare.None))
                {
                        ...
                        else
                        {
                            ...
                            foreach (txtTOxmlClassRetorno txtClass in oUniTxtToXml.cRetorno)
                            {
                                ...
                                if (File.Exists(vNomeArquivoDestino))
                                {
                                    ...
                                    //FileInfo oArqDestino = new FileInfo(vNomeArquivoDestino);
                                    //oArqDestino.Delete();
                                }
                                ///
                    ...
                    else
                    {
                        ...
                        foreach (txtTOxmlClassRetorno txtClass in oUniTxtToXml.cRetorno)
                        {
                            FileInfo odArquivo = new FileInfo(ConfiguracaoApp.vPastaXMLEnvio + "\\convertidos\\" + txtClass.XMLFileName);
                            odArquivo.Delete();
                        }
#endif
                    }
                    ...
                    File.WriteAllText(cArqErro, ccMessage, Encoding.Default);
#endif
                }
            }
        }
